// Generates a .pine file equivalent to a Custom MA Cross backtest config, so a winning
// combination can be promoted to live TradingView by pasting instead of hand-translating.
// Mirrors the structural conventions of strategies/ema-cross-tpsl.pine (ATR bracket) and
// strategies/ema-rsi-pct-tpsl.pine (RSI filter + % bracket) already in this repo.

// Mirrors customMaCross.ts's param `options` ordering — keep these two files in sync.
const MA_SMA = 1;
const TPSL_ATR = 1;
const TPSL_PCT = 2;
...
export function pineForCustomMaCross(params: Record<string, number>): string {
  ...
  push("// @version=6");
  push("// Generated from the crypto-bot backtesting tool's \"Custom MA Cross\" strategy.");
  ...
  push("");
  push("strategy(");
  push("  title              = \"Custom MA Cross (exported)\",");
  push("  overlay            = true,");
